...
class PositionalEncoding(nn.Module):
    ...
    In this case, the layer has no learnable parameters, since it is a simple
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0, :, 0::2] = torch.sin(position * div_term)   # 偶数列: sin
        pe[0, :, 1::2] = torch.cos(position * div_term)   # 奇数列: cos
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        sequence length, T is the target sequence length, and E is the
        embedding dimension.
        ...
        - output: Tensor of shape (N, S, E) giving the weighted combination of
          data in value according to the attention weights calculated using
          key and query.
        """
        N, S, E = query.shape
        N, T, E = value.shape
        ...
        #   2) The function torch.matmul allows you to do a batched matrix         #
        #      multiply. For example, you can do (N, H, T, E/H) by (N, H, E/H, T)  #
        #      to yield a shape (N, H, T, T). For more examples, see               #
        ...
        # 2) Scaled dot-product attention: (N, H, S, T)
        scores = torch.matmul(Q, K.transpose(-2, -1)) / scale

        # 3) Apply mask: positions where mask == 0 get -inf (zero weight after softmax)
        ...
        # 5) Weighted sum of values: (N, H, S, T) x (N, H, T, D) -> (N, H, S, D)
        out = torch.matmul(attn, V)

        # 6) Concatenate heads: (N, H, S, D) -> (N, S, E), then output projection
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        - memory: the sequence from the last layer of the encoder, of shape
          (N, S, D)
        ...
        ############################################################################
        # (1) Cross-attention block: query from decoder, key/value from encoder
        shortcut = tgt
...
class PatchEmbedding(nn.Module):
    """
    A layer that splits an image into patches and projects each patch to an
    embedding vector. Used as the input layer of a Vision Transformer (ViT).

    Inputs:
    - img_size: Integer representing the height/width of input image (assumes
      square image).
    ...
    def forward(self, x):
        ...
        ############################################################################
        n = self.img_size // self.patch_size  # number of patches per row/column

        # (N, C, H, W) -> (N, C, n, p, n, p) -> (N, n, n, C, p, p)
        out = x.reshape(N, C, n, self.patch_size, n, self.patch_size)
        out = out.permute(0, 2, 4, 1, 3, 5)

        # Flatten into patches: (N, num_patches, patch_dim)
