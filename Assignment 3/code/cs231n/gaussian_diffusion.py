...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # 1. Get the model prediction (either noise or x_start depending on objective)
        pred = self.model(x_t, t, model_kwargs=model_kwargs)

        # 2. Convert the prediction to x_start
        if self.objective == "pred_noise":
            x_start = self.predict_start_from_noise(x_t, t, pred)
        else:
            x_start = pred

        # 3. Clamp predicted x_start to the valid range [-1, 1]
        x_start = x_start.clamp(-1.0, 1.0)

        # 4. Get posterior q(x_{t-1} | x_t, x_0) and sample x_{t-1}
    ...
    def q_sample(self, x_start, t, noise):
        ...
        """
        # x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * noise
        x_t = extract(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start + extract(
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        # Model prediction (noise or x_start, depending on objective)
        pred = self.model(x_t, t, model_kwargs=model_kwargs)
...
def extract(a, t, x_shape):
    ...
    This function gathers the values from the coefficient tensor `a` according
    to the given timesteps `t` and reshapes them to match the required shape
    such that it supports broadcasting with the tensor of given shape `x_shape`.
    Args:
        a (torch.Tensor): A tensor of shape (T,), containing coefficient values
            for all timesteps.
        t (torch.Tensor): A tensor of shape (b,), representing the timesteps for
            each sample in the batch.
        x_shape (tuple): The shape of the input image tensor, usually (b, c, h, w).
    Returns:
        torch.Tensor: A tensor of shape (b, 1, 1, 1), containing the extracted
            coefficient values from `a` for corresponding timestep of each batch
            element, reshaped accordingly.
    """
