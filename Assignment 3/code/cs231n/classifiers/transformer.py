...
class CaptioningTransformer(nn.Module):
    ...
    Transformer decoder. The Transformer receives input vectors of size D,
    has a vocab size of V, works on sequences of length T, uses word vectors
    of dimension W, and operates on minibatches of size N.
    """

    def __init__(self, word_to_idx, input_dim, wordvec_dim,
                 num_heads=4, num_layers=2, max_length=50):
        """
        ...
        decoder_layer = TransformerDecoderLayer(input_dim=wordvec_dim,
                                                num_heads=num_heads)
    ...
    def forward(self, features, captions):
        """
        Given image features and caption tokens, return a distribution over
        the possible tokens for each timestep. Note that since the entire
        sequence of captions is provided all at once, we mask out future
        timesteps.
        ...
        ############################################################################
        # 1) Word embedding + positional encoding: (N, T) -> (N, T, W)
        cap = self.embedding(captions)
        cap = self.positional_encoding(cap)

        # Project image features to word-vector dimension and treat as a
        # length-1 sequence: (N, D) -> (N, 1, W)
        memory = self.visual_projection(features).unsqueeze(1)

        # 2) Causal mask (lower-triangular): mask out future timesteps, (T, T)
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device))

        # 3) Decoder with mask, then project to vocabulary scores
        dec = self.transformer(cap, memory, tgt_mask=tgt_mask)   # (N, T, W)
        scores = self.output(dec)                                # (N, T, V)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return scores
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=32, patch_size=8, in_channels=3, embed_dim=128,
                 num_layers=6, num_heads=4, dim_feedforward=256, num_classes=10,
                 dropout=0.1):
        """
        Inputs:
    ...
    def forward(self, x):
        ...
        ############################################################################
        # 1. Image -> patch sequence: (N, C, H, W) -> (N, num_patches, embed_dim)
        ...
        # 4. Average pool over the patch dimension
