...
class Unet(nn.Module):
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        # Feature maps saved for skip connections
        skips = []

        # 1. Downsampling: [ResnetBlock, ResnetBlock, Downsample] per level.
        #    Save the output of each ResnetBlock as a skip connection.
        for down_block in self.downs:
            x = down_block[0](x, context)  # ResnetBlock
            skips.append(x)
            x = down_block[1](x, context)  # ResnetBlock
            skips.append(x)
            x = down_block[2](x)  # Downsample (no context)

        # 2. Middle blocks
        x = self.mid_block1(x, context)
        x = self.mid_block2(x, context)

        # 3. Upsampling: [Upsample, ResnetBlock, ResnetBlock] per level.
        #    Concatenate with skip connections (reverse / LIFO order) before
        #    each ResnetBlock.
        for up_block in self.ups:
            x = up_block[0](x)  # Upsample (no context)
            x = torch.cat([x, skips.pop()], dim=1)  # concat skip -> 2 * dim_out
            x = up_block[1](x, context)  # ResnetBlock
            x = torch.cat([x, skips.pop()], dim=1)  # concat skip -> 2 * dim_out
            x = up_block[2](x, context)  # ResnetBlock
