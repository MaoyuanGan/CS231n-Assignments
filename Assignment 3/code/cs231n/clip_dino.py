...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images, class_texts, device):
    ...
    Args:
        ...
        images (List[np.ndarray]): A list of input images as NumPy arrays
            (H x W x C) uint8.
        class_texts (List[str]): A list of class label strings.
        device (torch.device): The device on which computation should be performed.

    Returns:
        List[str]: Predicted class label for each image, selected from class_texts.
    """
    pred_classes = []
    ############################################################################
    ...
    image_features = clip_model.encode_image(image_tensors)                 # (B, D)
    image_features = image_features / image_features.norm(dim=-1, keepdim=True)

    similarity = image_features @ text_features.T                           # (B, C)
    pred_idxs = similarity.argmax(dim=-1).cpu().tolist()
    pred_classes = [class_texts[i] for i in pred_idxs]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    return pred_classes
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # Encode ALL images once, up front (normalized so dot product == cosine sim).
        image_tensors = torch.stack(
            [clip_preprocess(Image.fromarray(img)) for img in images]
        ).to(device)                                                        # (B, 3, 224, 224)
        image_features = clip_model.encode_image(image_tensors)             # (B, D)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        similarity = (self.image_features @ text_features.T).squeeze(-1)    # (B,)
        top_indices = torch.topk(similarity, k=k).indices.cpu().tolist()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
# ---------------------------------------------------------------------------
# 4. DAVIS dataset helpers (given code — restored, no TODOs here)
# ---------------------------------------------------------------------------
class DavisDataset:
    def __init__(self):
...
class DINOSegmentation:
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        ############################################################################
        X_train = X_train.to(self.device)   # (N, D)
        Y_train = Y_train.to(self.device)   # (N,)
    ...
    def inference(self, X_test):
        ...
        pred_classes = None
        ############################################################################
        # TODO: Predict the segmentation class of each test feature vector.        #
        ############################################################################
        self.model.eval()
        X_test = X_test.to(self.device)
        logits = self.model(X_test)             # (N, num_classes)
        pred_classes = logits.argmax(dim=-1)    # (N,)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes