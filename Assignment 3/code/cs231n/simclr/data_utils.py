...
def compute_train_transform(seed=123456):
    ...
    Hint: look at available functions in torchvision.transforms
    """
    random.seed(seed)
    torch.random.manual_seed(seed)

    # Transformation that applies color jitter
    # with brightness=0.4, contrast=0.4, saturation=0.4, and hue=0.1
...
class CIFAR10Pair(CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            # 注意：必须独立调用两次，内部随机操作才会产生两个不同的增广视图
            ###########################################################################
