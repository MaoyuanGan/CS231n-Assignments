...
def train(model, data_loader, train_optimizer, epoch, epochs, batch_size=32,
    ...
    - data_loader: torch.utils.data.DataLoader object; loads in training data.
      You can assume the loaded data has been augmented.
    ...
    for data_pair in train_bar:
        ...
        ###########################################################################
        # 模型 forward 返回 (feature, out)：
        #   - feature: backbone 输出，用于 kNN 评估
        #   - out:     投影头输出，用于计算对比损失
        ###########################################################################
...
def train_val(model, data_loader, train_optimizer, epoch, epochs, device='cuda'):
    ...
    total_loss, total_correct_1, total_correct_5, total_num, data_bar = \
        0.0, 0.0, 0.0, 0, tqdm(data_loader)
        ...
        for data, target in data_bar:
            ...
            data_bar.set_description(
                '{} Epoch: [{}/{}] Loss: {:.4f} ACC@1: {:.2f}% ACC@5: {:.2f}%'.format(
                    'Train' if is_train else 'Test', epoch, epochs,
...
def test(model, memory_data_loader, test_data_loader, epoch, epochs, c,
         temperature=0.5, k=200, device='cuda'):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for data, _, target in test_bar:
            ...
            one_hot_label = torch.zeros(data.size(0) * k, c, device=device)  # [B*K, C]
