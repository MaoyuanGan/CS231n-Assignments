...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # **the width and height of the input are preserved**. Take a look         #
        # at the start of the loss() function to see how that happens.             #
        ############################################################################
        C, H, W = input_dim
        ...
        # 卷积使用 same padding 保持空间尺寸，经 2x2 max pool 后尺寸减半
        pool_out = (H // 2) * (W // 2)

        # 隐藏全连接层: W2 形状 (F * pool_out, hidden_dim)
        self.params["W2"] = weight_scale * np.random.randn(
            num_filters * pool_out, hidden_dim
        )
        self.params["b2"] = np.zeros(hidden_dim)

        # 输出全连接层: W3 形状 (hidden_dim, num_classes)
