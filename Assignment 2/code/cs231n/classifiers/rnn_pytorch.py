...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # will be the START token, and the first element of captions_out will be
        # the first word.
        ...
        ############################################################################
        # (1) Image features -> initial hidden state: (N, H)
        h0 = features.to(W_proj.dtype) @ W_proj + b_proj

        # (2) Word indices -> word vectors: (N, T, W)
        embeddings = word_embedding_forward(captions_in, W_embed)

        # (3) Run the RNN/LSTM over the sequence: (N, T, H)
        if self.cell_type == "rnn":
            hidden = rnn_forward(embeddings, h0, Wx, Wh, b)
        else:  # lstm
            hidden = lstm_forward(embeddings, h0, Wx, Wh, b)

        # (4) Hidden states -> vocabulary scores at every timestep: (N, T, V)
        scores = temporal_affine_forward(hidden, W_vocab, b_vocab)

        # (5) Temporal softmax loss, ignoring positions where the output word
        #     is <NULL>
    ...
    def sample(self, features, max_length=30):
        ...
        highest score as the next word.

        The initial hidden state is computed by applying an affine transform to
        the input image features, and the initial word is the <START> token.
        ...
        ###########################################################################
        with torch.no_grad():
            # Initial hidden state from image features: (N, H)
            h = features.to(W_proj.dtype) @ W_proj + b_proj
            # For LSTM, keep track of the cell state; init to zeros
            c = torch.zeros_like(h) if self.cell_type == "lstm" else None

            # The first input word is the <START> token
            word = torch.full((N,), self._start, dtype=torch.long)

            for t in range(max_length):
                # (1) Embed the previous word: (N, W)
                emb = W_embed[word]

                # (2) One RNN/LSTM step: (N, H)
                if self.cell_type == "rnn":
                    h = rnn_step_forward(emb, h, Wx, Wh, b)
                else:  # lstm
                    h, c = lstm_step_forward(emb, h, c, Wx, Wh, b)

                # (3) Scores for all vocabulary words: (N, V)
                scores = h @ W_vocab + b_vocab

                # (4) Greedy pick: highest-scoring word becomes the next word
