...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # the self.params dictionary. (已完成)                                     #
        ############################################################################
        ...
        if self.normalization == "batchnorm" or self.normalization == "layernorm":
            for i in range(self.num_layers - 1):        # 只有隐藏层带 norm
    ...
    def loss(self, X, y=None):
        ...
        scores = None
        ############################################################################
        # TODO: Implement the forward pass for the fully connected net. (已完成)   #
        ############################################################################
        caches = []
        h = X
        # 隐藏层：{affine - [batch/layer norm] - relu - [dropout]} × (L - 1)
        for i in range(1, self.num_layers):
            h, fc_cache = affine_forward(
                h, self.params["W%d" % i], self.params["b%d" % i]
            )

            norm_cache = None
            if self.normalization == "batchnorm":
                h, norm_cache = batchnorm_forward(
                    h,
                ...
                h, norm_cache = layernorm_forward(
                    h,
                    self.params["gamma%d" % i],
                    self.params["beta%d" % i],
                    self.bn_params[i - 1],
        ...
        # 最后一层：affine - softmax
        L = self.num_layers
        scores, last_fc_cache = affine_forward(
            h, self.params["W%d" % L], self.params["b%d" % L]
        )
        ...
        loss, grads = 0.0, {}
        ############################################################################
        # TODO: Implement the backward pass for the fully connected net. (已完成)  #
        ############################################################################
        # softmax 数据损失
        loss, dscores = softmax_loss(scores, y)

        # L2 正则（含 0.5 系数；gamma/beta 不做正则）
