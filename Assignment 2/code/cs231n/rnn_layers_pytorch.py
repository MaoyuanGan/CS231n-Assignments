...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    """Run the forward pass for a single timestep of a vanilla RNN using a tanh
    activation function.

    The input data has dimension D, the hidden state has dimension H, and
    the minibatch is of size N.
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = torch.stack(hidden_states, dim=1)  # (N, T, H)
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return h
...
def word_embedding_forward(x, W):
    """Forward pass for word embeddings. We operate on minibatches
    of size N where each sequence has length T. We assume a vocabulary of V
    words, assigning each word to a vector of dimension D.
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i = sigmoid(a[:, 0 * H:1 * H])               # input gate
    f = sigmoid(a[:, 1 * H:2 * H])               # forget gate
    o = sigmoid(a[:, 2 * H:3 * H])               # output gate
    g = torch.tanh(a[:, 3 * H:4 * H])            # block input
    next_c = f * prev_c + i * g
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    state is set to zero. Also note that the cell state is not returned; it is an
    internal variable to the LSTM and is not accessed from outside.
    ...
    N, T, _ = x.shape
    prev_h = h0
    prev_c = torch.zeros_like(h0)                # initial cell state is zero
    hidden_states = []
    for t in range(T):
...
def temporal_affine_forward(x, w, b):
    ...
    length T. We use an affine function to transform each of those vectors into
